...
def test_force_sync_button(app_page):
    ...
    shell.open_tools()
    force_btn = app_page.locator(".tools-item").filter(has_text="Force Sync")
...
def test_export_data_button(app_page):
    ...
    with app_page.expect_download(timeout=5000) as download_info:
        app_page.locator(".tools-item").filter(has_text="Export All Data").click()
...
def test_debug_log_button(app_page):
    ...
    shell.open_tools()
    with app_page.expect_download(timeout=5000) as download_info:
        app_page.locator(".tools-item").filter(has_text="Save Debug Log").click()
    download = download_info.value
    assert download.suggested_filename.endswith(".txt")


def test_tools_close(app_page):
    """Closing tools menu returns to the module view."""
    shell = AppShellPage(app_page)
    shell.open_tools()
