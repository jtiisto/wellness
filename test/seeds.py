...
def seed_coach_plan(conn, *, today, yesterday=None, supersets=False,
    ...
    if supersets:
        ...
        """, (s1, b2))
    if prescription:
        # A strength exercise carrying the full structured prescription (tempo +
        # target_rpe + target_load) and NO guidance_note, so the UI shows the
        # consolidated prescription line. KB Goblet Squat above keeps its legacy
        # inline "Tempo 3-1-1" guidance_note (no structured fields) so both the
        # new line and the historical note are exercised.
        cursor.execute("""
            INSERT INTO planned_exercises
            (session_id, block_id, exercise_key, position, name, exercise_type,
             target_sets, target_reps, tempo, target_rpe, target_load)
            VALUES (?, ?, 'ex_tempo', 3, 'Front Squat', 'strength', 4, '6',
                    '3-1-2-0', '6-7', '70%')
        """, (s1, b2))
