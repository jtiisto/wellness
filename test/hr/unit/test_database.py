"""Unit tests for HR database initialization (migration 1 baseline)."""
import sqlite3

import pytest

from modules.db import DbAccessor, get_db
...
class TestInitDatabase:
    def test_creates_all_tables(self, tmp_path):
        ...
        assert {"intervals", "set_events", "sessions"} <= tables
    ...
    def test_creates_indexes(self, tmp_path):
        ...
        assert {"idx_intervals_session", "idx_set_events_date"} <= indexes
...
class TestSchema:
    ...
    def test_no_foreign_keys_between_tables(self, tmp_path):
        ...
        _fresh(db_path)
        with get_db(db_path) as conn:
            for table in ("intervals", "set_events", "sessions"):
