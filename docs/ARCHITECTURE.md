...
**Tracker polarity.** `polarity` (the canonical `trackers.polarity` column) is an optional `'positive' | 'negative' | 'neutral'` label (absent = unspecified/neutral) describing a tracker's valence — e.g. a habit to build (`positive`), a behavior to avoid (`negative`), or a neutral measurement. The collapsed-category on-track summary now reads it (below) and the MCP adherence tool is the other consumer; it stays out of the grid rows and completion logic. It is kept orthogonal to scheduling (meaning vs. routine).
...
**Collapsed-category summary.** Each collapsed category pill carries a schedule- and polarity-aware on-track rollup (`categorySummary` / `formatCategorySummary` in `public/js/journal/utils.js`): it counts only trackers *expected* that day (not-expected ≠ missed) and buckets each by its single-day `dayStatus`, so a checked positive habit, an avoided (un-logged) negative, and a value that meets its target all read as "on track" ("3 of 4 on track" / "All on track"), while a pure-neutral category is worded "logged". It is a single-day view (no naive completion count); windowed adherence stays in the MCP. Each expanded row also carries a **7-day dot row** (`recentDayStates`): the same single-day `dayStatus` repeated across the last seven local days (off-schedule days muted, today marked), giving recent texture without duplicating windowed adherence — which remains MCP-only.
