...
**Tracker `meta_json` — free-form settings passthrough.** Any tracker field the sync protocol doesn't own (`_TRACKER_RESERVED_KEYS` in `journal.py`) is serialized into `meta_json` on upload and merged back onto the tracker on read. The server never interprets it — `unit`, `defaultValue`, `accumulator`, `scheduleHistory`, `polarity`, and the legacy `frequency`/`weeklyDay` all ride this passthrough with full optimistic-concurrency arbitration, archiving, and delta for free, because the tracker row is the sync unit. New settings are additive: no wire/schema change and no LocalForage schema bump.
...
The schedule is a pure client-side derivation — the server and MCP see only opaque JSON. Adherence/completion scoring is intentionally *not* built on it: `get_journal_summary`'s completion rate stays entries-based. Segments are objects (not bare arrays) so a future `kind` discriminator (rolling multi-week cycles, weekly counts) can be added without another migration.

**Tracker polarity.** `meta_json.polarity` is an optional `'positive' | 'negative' | 'neutral'` label (absent = unspecified/neutral) describing a tracker's valence — e.g. a habit to build (`positive`), a behavior to avoid (`negative`), or a neutral measurement. It is stored-only: nothing in the grid, summary, MCP, or any completion logic reads it yet; the journal entry-screen redesign is the intended first consumer. It is kept orthogonal to scheduling (meaning vs. routine).
