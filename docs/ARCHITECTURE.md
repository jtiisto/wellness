...
**Canonical storage.** `scheduleHistory` and `polarity` are **promoted out of the `meta_json` blob into protocol-owned columns** — `trackers.schedule_json` (the segments array as JSON) and `trackers.polarity` (migration 3; mirrored on `trackers_archive`; existing rows backfilled from `meta_json`). The server captures them from the tracker upload into the columns on every upsert, archives them with the prior row, and emits them as top-level `scheduleHistory` / `polarity` fields on read — so the **sync wire shape is unchanged** (they were already top-level in the tracker dict; only the storage location moved). During the transition they stay **dual-stored** in `meta_json` too (not yet added to `_TRACKER_RESERVED_KEYS`), so older readers keep working and rollback needs no data step; a later phase adds them to the reserved keys and strips the `meta_json` copy. This makes the schedule and polarity canonical server data so the journal MCP can compute schedule adherence server-side. Adherence is still deliberately kept out of `get_journal_summary`, whose completion rate stays entries-based.

**Tracker polarity.** `polarity` (the canonical `trackers.polarity` column, dual-stored in `meta_json` during the transition) is an optional `'positive' | 'negative' | 'neutral'` label (absent = unspecified/neutral) describing a tracker's valence — e.g. a habit to build (`positive`), a behavior to avoid (`negative`), or a neutral measurement. It is not read by the grid, summary, or completion logic; the journal entry-screen redesign and MCP adherence are the intended consumers. It is kept orthogonal to scheduling (meaning vs. routine).
