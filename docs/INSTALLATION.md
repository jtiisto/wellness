...
What gets deployed is driven entirely by **`bin/deploy.manifest`** (the single source of truth): `src/`, `public/`, `mcp_servers/`, `requirements.txt`, and a curated allowlist of `bin/` scripts (`server.sh`, the workout hooks, `backup-databases.sh`). Dev tooling, `docs/`, `plans/`, `test/`, `githooks/`, and the `android/` client tree are intentionally excluded. To add or remove a deployed file, edit the manifest — the guard test `test/test_deploy_manifest.py` fails until every tracked top-level entry and `bin/` script is classified there, so nothing ships (or fails to ship) by accident. `data/` deliberately never ships — the server creates its databases on first start, keeping rsync away from live production data. The optional LLM directory argument specifies where Claude Code CLI runs analysis queries (it must contain `CLAUDE.md` and `.claude/` with MCP configs).
