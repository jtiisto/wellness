...
- [x] Idempotent migration in `init_database` — `try: ALTER TABLE exercise_logs DROP COLUMN completed; except sqlite3.OperationalError: pass`. **Retired 2026-05-29** after the migration applied to prod (one-shot; the `CREATE TABLE` already omits the column for fresh DBs). A stray legacy column is harmless (writer omits it, reads ignore it) — covered by `TestExerciseLogsSchema`.
