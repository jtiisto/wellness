...
- **Welded category card (journal)**: header band on `band` (radius 8 top, hairline sides), rows on `card` stitched below (hairline sides+bottom, last row radius 8 bottom); collapsed = standalone band pill (all corners 8); chevron 14dp rotating. Right-aligned rollup = the **signal ring cluster** (2026-08-14, spec in journal-ui.md): segmented 18dp habit ring (met=success, partial=accent, not-yet=faint track, state-sorted), avoidance center (quiet hoop held / warning fill broken; standalone 8dp when no ring), faint diamond + `micro` count for observations. Replaces the text badge in both band states; honesty mapping (restraint never success-colored) carries through.
