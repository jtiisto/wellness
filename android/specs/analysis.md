...
1. **Manual location field, no geolocation** (plan-mandated). A query with `accepts_location: true` renders an optional single-line `WellnessDenseField` inline on the expanded row; the ViewModel owns per-query field text (never persisted). Blank → key **omitted** from the POST body. No location permission, no Nominatim. (PWA: silent geolocation; the field matches `ARCHITECTURE.md:689`'s documented intent.) *Round 4: the card is a hairline-separated row and the field takes the `NAKED` skin with its own mono-caps label — visual clauses now governed by [logbook-design-system.md](logbook-design-system.md) ("Run view and chrome"), which supersedes them here. The value stays in the **body** face, not mono: a town is words, and principle 3 spends mono on numerals.*
...
10. **Delete confirmation** = M3 AlertDialog. *Round 4: it inherits the Logbook M3 adapter — paper, ink, 2dp — with no per-callsite colour work, and the destructive verb keeps its own ink (Logbook owns no error token; the sentence above the button is what says what is destroyed). "Destructive-styled" is superseded by [logbook-design-system.md](logbook-design-system.md).*
...
- Rendered (`ReportBody`) as **ink mark + mono caps word**: filled dot `OK`, half dot `WATCH`, open dot + mono `!` `ACT`. The mark is inline content over a blank placeholder and the word is the text, so a screen reader announces the verdict once. *Round 4: the marks are `InkJudgment` in `core/ui` — the design system's own judgment vocabulary, shared with the history rows — resolved from the Logbook palette. (P1a drew them from whichever theme was current, because this destination was still Graphite.)*
...
- `Link` keeps its destination; rendered as `label (destination)` in the quiet tone (Round 4: ink-soft **mono**, because a URL is a literal rather than prose) — inert, not clickable, but the URL is never silently dropped (health reports may carry meaningful references). Images → `Text("[image: {alt}] ({destination})")`.
...
Composables: `ReportBody(blocks)`, the query row (inline location field, disabled on `submitInFlight`), `ProgressView` (1 s clock keyed on the Loaded `createdAt` — Stub renders 0s; Cancel; the UNKNOWN-ceiling state with re-check), `ReportViewScreen` (FAILED shows `errorMessage` + Try Again per deviation 7), `HistoryList` (delete ✕ hidden for non-terminal rows — parity; confirm dialog).

**Round 4 supersedes this paragraph's visual half** ([logbook-design-system.md](logbook-design-system.md), "Components — Analysis & Tools"): query rows are text-only and the **icon map is retired** — `queryIcon` and its test are gone, and `AnalysisQueryDto.icon` is now decoded and ignored rather than drawn; the progress spinner is a mono elapsed clock over a hairline ink sweep; history rows judge in shapes (filled dot done, open dot + `!` failed, faint open dot in flight) instead of a coloured status word, which survives only for rows that are still on their way. The presentation derivations behind all of it — header eyebrow, section qualifiers, the history mark and its spoken word, the report's own top heading level — live in `AnalysisUiLogic` and are pinned by `AnalysisUiLogicTest`.
