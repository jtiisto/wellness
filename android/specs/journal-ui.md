...
- `../../public/js/journal/store.js` — UI-facing surface (`updateEntry` merge semantics, `toggleCategoryExpanded`, `markValueUpdated`)
...
7. Emoji glyphs → Material icons, each with a content description (chevron, delete, checkbox state, slider value — the emoji carried textual cues we must not lose; the lock badge retired with the day lock, 2026-08-18). Category sort unified to plain `compareTo` at BOTH levels (the PWA's category keys already sort UTF-16 lexicographic via comparator-free `Array.sort()`; only tracker *names* used `localeCompare` — the unification changes name ordering for non-ASCII names; pinned by a test with an accented name).
...
10. *(v3, from implementation)* **Untouched tracker save is a true no-op**: `updateTrackerAndMarkDirty` compares the transformed row to the stored one and skips write + dirty when identical. The PWA always writes and dirties — which, before the day lock retired (2026-08-18), phantom-locked past dates behind an empty upload; the no-op save still matters for not queueing empty uploads. One documented wrinkle: a pre-accumulator-era tracker's first untouched save legitimately writes (the form fills in missing quantifiable keys) — every save after is a no-op.
11. *(v3, from implementation)* **Blank numeric input clears the value to ABSENT** (`Set(null)` → SQL NULL), not the PWA's explicit null — re-ticking the checkbox then reseeds the default. Unparseable non-blank input restores the displayed value with no write. Upload serialization is identical either way (`"value": null`). Every other clear takes the same two forms: the uncheck retracting its seed (2026-08-18) and the blank note (2026-08-19).
...
- **`mergeEntry(date, trackerId, patch: EntryPatch)`** — the widgets' single write path. `EntryPatch` is presence-aware per field: `value: Field<JsonElement?> { Unchanged | Set(v) }`, `completed: Field<Boolean?>` — `Set(null)` is an explicit clear, `Unchanged` preserves the stored column; both-`Unchanged` is a no-op (no write, no dirty). Implemented as ONE atomic DAO transaction (read-current + apply + mark **entry** dirty — never the tracker), then `afterLocalChange()`. Phase 2's `updateEntry`/`setEntryCompleted` remain but the checkbox's "check + write default" case MUST be one `mergeEntry` call (`{value: Set(default), completed: Set(true)}`), not two.
- **Entry presence semantics (load-bearing)**: two separate questions, and they answer differently.
  - **Storage** — the PWA distinguishes `entry.value === undefined` (absent) from explicit `null`. Room mirrors it: `valueJson` SQL NULL = absent; `valueJson = "null"` (encoded JsonNull) = explicitly null. **Unchanged**: this distinction is preserved end to end at the storage and wire layers.
  - **Visibility** — `shouldShowTracker` keys off **raw row existence**. An all-empty row still shows an off-schedule tracker, deliberately: the row has to stay reachable so the entry can be found and cleared.
  - **Judgment** — `dayStatus`/`hasEntry`/the tally/the week marks key off `EntryDto?.countsAsLogged()`: `completed == true || hasValue()`. An uncheck writes `{completed: false}` and keeps the row (there is no entry delete on the wire), so judging row existence made an uncheck irretractable — the day stayed "broken" forever. An all-empty row now judges exactly like no row; a written value keeps the row judged even with the box cleared (the value is the assertion; blanking the field retracts it). One predicate, twinned with `entryCountsAsLogged` (PWA) and `entry_present` (server) — see docs/ARCHITECTURE.md "Entry presence for judgment".
...
- **Numeric display collapses integral values** (2026-08-14): every user-facing number — the value field included — renders through `formatJournalNumber`, so `5.0` displays as `5` and genuine fractions keep their digits. The trap this closes: local writes integer-collapse via `journalNumberJson`, but the server's REAL column returns integers as Python floats, so a delta-delivered value arrives as `5.0` and previously displayed that way. Units are chosen so half-units aren't needed; the display never invents precision the user didn't type.
- **No day lock.** Every day the strip offers is editable, always. (Retired 2026-08-18 — see Behavior rule 1.) The store keeps no editability query and the DAO no dirty-tracker count; entry writes still mark only the *entry* dirty, which the sync engine reads.
...
MVI with StateFlow:
- `JournalViewModel`: `UiState(dateStrip: List<DayCell>, selectedDate, groups: List<CategoryGroup(name, expanded, summary?, trackers: List<TrackerRowState>)>, syncStatus)` — `TrackerRowState` carries everything a row renders (widget type + current/displayed value + committed flag + targetProgress? + lastUpdatedCaption? + dotRow states). Derivation is a pure `buildJournalUiState(...)` function over (trackers, day entries, selected date, prefs, now) — unit-testable without Compose.
...
1. **Date strip**: fixed trailing 7 days ending today, **every day always tappable**. Selection is UI state only (survives via ViewModel, not persisted). *(Superseded 2026-08-18: the dirty-tracker day lock is retired on both clients. It did what it was designed to do — a pending config edit locked every non-today day until upload — but offline that stranded the user with a read-only week, and fixing a forgotten log must always stay possible. The lock badge, the disabled rows, the `dayEditable` state field and the LOCKED eyebrow voice all retire with it.)*
...
4. **Widgets**: checkbox for all types except note; checking writes the default value **only when the entry's value is ABSENT** (`valueJson` SQL NULL or no row — an explicit stored null does not trigger it; PWA: `entry.value === undefined`), as one atomic `mergeEntry` (quantifiable: `defaultValue`; evaluation: `defaultValue ?? 50`). **Unchecking retracts the seed** *(2026-08-18, device pass round 2)*: a stored value numerically equal to what a check would seed clears with the uncheck (one atomic `{value: clear, completed: false}` — Android clears to ABSENT, the PWA to its explicit-null clear convention; judgment reads both as no value), because under the emptiness rule a leftover seed kept the row counting as logged after the user took it back. A value that differs from the seed survives the uncheck — typed and accumulated numbers are retracted by blanking the field, never by the checkbox. Quantifiable numeric field: commit on focus-loss or IME Done; parse the string — invalid input restores the displayed value (no write, no error state); numeric equality after parse (`"1"` == `1.0` == displayed default) is a no-op with no stamp; a real change writes `{value: Set}` only (never touches `completed`) + stamps. Accumulator adds to `(coerced displayed value || 0)`, allows negatives, silently closes on 0/invalid, writes `{value: Set}` + stamps. Evaluation slider: writes on every `onValueChange` step (live, PWA `onInput` parity — the 2.5 s scheduler debounce absorbs the burst; single-flight already guards overlap), `{value: Set}` only. Note textarea: each edit writes value and checkbox atomically — a non-blank edit writes `{value: Set(text), completed: Set(true)}`, so typing commits. **A blank edit is a retraction** *(2026-08-19, device pass round 3)*: it writes `{value: clear, completed: Set(false)}` — the text clears back to *no value* rather than being stored as `""`. Writing the blank string back was the bug: under the emptiness rule `""` is a value, so a cleared note kept the row counting as logged forever and no gesture in the UI could take it back. This is the numeric field's own convention (emptying the field clears the value to absent) extended to text — Android clears to ABSENT, the PWA to its explicit-null clear convention, and judgment reads both as no value. Blankness stays each platform's idiomatic test — Kotlin `isBlank()`, JS `trim() !== ''`, the same tests that already gated `completed` here; they can only disagree on exotic unicode whitespace, and that theoretical divergence is accepted rather than papered over with a shared definition (the same stance the cross-stack number/date conventions take). One rendering consequence, PWA-only: its textarea is controlled by the stored value, so a whitespace-only edit clears the field as it is typed; Android's field holds local text while focused and adopts the store's value on blur. Rows that already store `""` from before this rule are **left alone** — no migration, no read-time rewrite: they keep counting as logged until the field is blanked once more, which now sticks.
5. **Uncommitted styling** (`completed !== true`): value controls ghosted, dot row NOT ghosted. Every row is enabled on every day (see rule 1).
...
| `JournalUiStateTest` (:feature:journal) | `buildJournalUiState`: strip shape, judgment-presence rule, visibility + empty states, group/summary assembly, row state derivation (displayed-value fallback chain, committed flag, caption formatting), dot-row wiring |
