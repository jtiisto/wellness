...
2. **Rendering through Graphite Signal** (`ChartTheme`, `LocalWellnessPalette`, module accent), not the PWA's `.trends-*` CSS. Geometry parity, visuals native. Series colors: primary = module accent (`ChartTheme.line`), secondary/alt = `ChartTheme.altLine`; stack/band/status colors resolved from the palette at implementation time following `specs/design-system.md`.
