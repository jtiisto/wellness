...
| Test class | Pins |
|---|---|
| `CoachSyncLogicTest` (:core:data) | all 27 `coach-sync-logic.test.js` cases 1:1 (fixtures in the JS tests are synthetic — transcribe verbatim). **v2.2 per-record arbitration**: an untouched record adopts the verdict; a genuinely re-edited one keeps content and advances; one day carrying both at once; a record created mid-sync stays unarbitrated; untouched vs re-edited `session_feedback` against the day token; a settled re-add sheds `_readd`; `uploadedLogs == null` grants no verdicts |
